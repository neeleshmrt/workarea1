...
select
        to_char(to_date(tday||' '||tmod*&v_secs,'YYMMDD SSSSS'),'DD-MON  HH24:MI:SS') TIME,
        --samples npts,
        round(total/&v_secs,1) Active_SESS,
        round(cpu/&v_secs,1) CPU,
        round(io/&v_secs,1) IO,
        round(IO_DB_SEQ/&v_secs,1) DB_SEQ,
        round(IO_DB_SCATTER/&v_secs,1) DB_SCATTER,
        round(IO_DB_PARALLEL/&v_secs,1) DB_PARALLEL,
...
from (
   select
     ...
     , sum(decode(session_type,'FOREGROUND',
      ...
      ,sum(decode(session_type,'FOREGROUND',
		 decode(event,'db file sequential read',10, 0))) IO_DB_SEQ                               
      ,sum(decode(session_type,'FOREGROUND',
		 decode(event,'db file scattered read',10, 0))) IO_DB_SCATTER
      ,sum(decode(session_type,'FOREGROUND',
		 decode(event,'db file parallel read',10, 0))) IO_DB_PARALLEL		          
