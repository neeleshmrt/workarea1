	...
	<xsl:choose>
		<xsl:when test="../@id">
			<h2>
				...
				&#160;<a href="#top">overall</a>&#160;<a href="#stm{$id}">current</a>&#160;
			</h2>
		</xsl:when>
		<xsl:otherwise>
			<h2 id="top">
...
<xsl:template match="event">
	...
	<xsl:choose>
		<xsl:when test="../../@id">
			<h3 id="{$name}-stm{$id}">
				<xsl:value-of select="@name"/>
				&#160;<a href="#{$name}-stm">overall</a><xsl:if test="../../@id">&#160;<a href="#stm{$id}">current</a></xsl:if>&#160;
			</h3>
		</xsl:when>
		<xsl:otherwise>
			<h3 id="{$name}-stm">
				<xsl:value-of select="@name"/>
				&#160;<a href="#top">overall</a><xsl:if test="../../@id">&#160;<a href="#stm{$id}">current</a></xsl:if>&#160;
			</h3>
		</xsl:otherwise>				
	</xsl:choose>
	</xsl:if>
