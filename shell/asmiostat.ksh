...
###########################################################################################################
### USAGE  
###             asmiostat.ksh -g DATA 600 144 >> /home/oracle/output_dir/asmiostat_DATA.log
##########################################################################################################
...
export ORACLE_SID=+`grep ASM /etc/oratab|awk -F: '{print $1}'|awk -F+ '{print $2}'`
#export ORACLE_HOME=`grep ASM /etc/oratab|grep -v^#|/bin/awk -F: '{print $2}'|/bin/awk -F: '{print $1}'`
export ORACLE_HOME=`grep ASM /etc/oratab|grep -v '^#'|/bin/awk -F: '{print $2}'|/bin/awk -F: '{print $1}'`
#export ORAENV_ASK=NO
export PATH=/usr/local/bin:$ORACLE_HOME/bin:$PATH
#. /usr/local/bin/oraenv $ORACLE_SID 2>/dev/null 1&>2
...
$asmcmdPath pwd  | grep -q '^\+$' || fatalError "$ORACLE_SID is not an ASM instance"
}
